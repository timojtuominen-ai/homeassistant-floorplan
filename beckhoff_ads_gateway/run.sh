...
export LIST_SYMBOLS="$(bashio::config 'list_symbols')"
export SYMBOL_FILTER="$(bashio::config 'symbol_filter')"
export MAX_SYMBOLS="$(bashio::config 'max_symbols')"
...
bashio::log.info "Starting Beckhoff ADS Gateway v0.3.1"
bashio::log.info "PLC ${PLC_IP}, AMS ${PLC_AMS_NET_ID}, port ${PLC_ADS_PORT}"
bashio::log.info "Local AMS Net ID ${LOCAL_AMS_NET_ID}"
bashio::log.info "Direct symbol test: ${ADS_SYMBOL}"
bashio::log.info "Symbol browser: ${LIST_SYMBOLS}, filter: '${SYMBOL_FILTER}', max: ${MAX_SYMBOLS}"
