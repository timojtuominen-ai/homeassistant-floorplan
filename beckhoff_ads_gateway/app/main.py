...
def pulse_light_command(plc, light, command):
    ...
    # arrLampCommands is TcBA's internal PLC command array.
    # Keep the opposite command low, create a clean rising edge on the requested
    # command, verify that the PLC symbol really became TRUE, then clear it.
    ...
    LOGGER.info(
        "TcBA command asserted: %s -> %s, symbol=%s, readback=%s",
        light["id"], command, symbol, readback_high
    )
    time.sleep(pulse_ms / 1000.0)
    plc.write_by_name(symbol, False, pyads.PLCTYPE_BOOL)
    readback_low = bool(plc.read_by_name(symbol, pyads.PLCTYPE_BOOL))
    LOGGER.info(
        "TcBA light pulse completed: %s -> %s (%d ms), cleared_readback=%s",
        light["id"], command, pulse_ms, readback_low
    )
